...
class PPF_infer_Dataset(data.Dataset):  
    ...
    def get_fs_net_scale_part(self, c, urdf_id):
        ...
        if c == 'laptop':
            ...
            for i in range(self.num_parts):
                # '/home/mxh24/codes/a_datas/dataset1/laptop/train/laptop/urdf/10356/part_point_sample_rest/0.xyz'
    ...
    def get_frame(self, choose_frame_annotation,choose_urdf_id, norm_q, norm_r):
        ...
        for i in range(self.num_parts):
            ...
            #     cano_pcd.transform(np.linalg.inv(part_target_rt[0]))  
            # else:
            #     cano_pcd.transform(np.linalg.inv(part_target_rt[1])) 
        ...
        q_homo = np.concatenate([q_iner, np.ones((len(q_iner), 1))], axis=1)  # [N, 4]
        for i in range(self.num_parts):          
            part_target_rt[i][:3,3] += part_target_rt[i][:3,:3] @ centers_pt[i].reshape(3,)
           
            npcs_q_homo.append(np.squeeze(np.linalg.inv(part_target_rt[i]) @ ori_gt_rt[i] @ q_homo.T))  # (4,)

        
        # vis_cloud(cloud_canonical=cloud)
        part_cls = part_mask.flatten()[choose]
        
       
        clouds_part_cano = copy.deepcopy(clouds)
        ...
        for i in range(self.num_parts):
            ...
            clouds[i] = o3d_trans(clouds_part_cano[i], part_target_rt[i])  

      
        targets_scale = []
        s_ = [np.array(self.cfg.scale_mean_base), np.array(self.cfg.scale_mean_child)]
        for i in range(self.num_parts):
            bounds = [np.min(clouds_part_cano[i],axis=0),np.max(clouds_part_cano[i],axis=0)]
            sc = s_[i] 
            bound = bounds[1] - bounds[0]
            targets_scale.append(bound) # (np.log((bound / 2).astype(np.float32)) - np.log(sc))
        ...
        out_dic = {}
        out_dic['clouds'] = np.stack(clouds).astype(np.float32)
        out_dic['clouds_part_cano'] = np.stack(clouds_part_cano).astype(np.float32)

        out_dic['part_target_rt'] = part_target_rt  
        out_dic['part_target_r'] = part_target_rt[:,:3,:3]
        out_dic['part_target_t'] = part_target_rt[:,:3,-1]
        out_dic['homo_q'] = np.stack(npcs_q_homo).astype(np.float32)
        out_dic['targets_scale'] = np.concatenate(targets_scale,axis=-1,dtype=np.float32)
        ...
        out_dic['joint_state'] = joint_state
        out_dic['sort_part'] = sort_part
        
        ############################################################
        #### use to segmentation
        label = np.zeros((len(cloud)),dtype=np.int64)
        label[:len(clouds[0])] = 0
        label[len(clouds[0]):] = 1
        cls_label = label
        cls_camera_pts = np.concatenate(camera_clouds, axis=0) 
        ...
        if cls_random:
            ...
            cls_camera_pts = cls_camera_pts[cls_mask]
            cls_label = label[cls_mask]
        else:
            cls_camera_pts = cls_camera_pts
            cls_label = label
    ...
    def __len__(self):
        return min(500, self.num_samples)
