...
# from dataset_train import PM_Dataset
...
class PPF_infer_Dataset(data.Dataset):  
    ...
    def __init__(self, cfg, mode, data_root,
        ...
        self.data_root = data_root  # '/home/xxx/codes/a_datas/dataset1'
        self.mode = mode
        ...
        valid_flag_path = osp.join(self.data_root, self.CLASSES[cate_id], 'train.txt'
            if mode == 'train' else 'test.txt') # '/home/xxx/codes/a_datas/ArtImage/laptop/train.txt'
        ...
        for video in sorted(os.listdir(self.data_dir)): # ArtImage/laptop/train
            ...
            #     continue
            video_path = osp.join(self.data_dir, video) # '/home/xxx/codes/a_datas/ArtImage/laptop/train/00000'
    ...
    def get_frame(self, choose_frame_annotation,choose_urdf_id):
        ...
        for i in range(self.num_parts):
            ...
            if len(part_seg) == 0:
                ...
                return self.get_frame(self.obj_annotation_list[rd_index],self.obj_urdf_id_list[rd_index])
        ...
        ori_gt_rt = copy.deepcopy(part_target_rt)  # 转到相机空间的变换矩阵
        
        centers_pt = [(np.min(c_pts,axis=0) + np.max(c_pts,axis=0)) / 2 for c_pts in cano_clouds]
        ...
        npcs_q_homo = []
        q_iner = joint_loc + 0.01*joint_axis
        q_homo = np.concatenate([q_iner, np.ones((len(q_iner), 1))], axis=1)  # NAOCS下的轴点 [numparts-1, 4]
        clouds_part_cano =  copy.deepcopy(cano_clouds)
        for i in range(self.num_parts):                
            part_target_rt[i][:3,3] += part_target_rt[i][:3,:3] @ centers_pt[i].reshape(3,) # NPCS到相机空间的变换矩阵
            clouds_part_cano[i] -= centers_pt[i]
        
        np.savetxt('/home/xxx/codes/PPF_Tracker_release/pc_out.txt',np.concatenate(clouds_part_cano,axis=0))
        # print(q_homo[0],q_homo[-1])
        # part_target_rt.shape = [numparts,4,4]
        for j in range(self.num_parts-1):   
            prev_q = (np.linalg.inv(part_target_rt[0])   @ ori_gt_rt[0]   @ q_homo[j].T)  # [4,]
            next_q = (np.linalg.inv(part_target_rt[j+1]) @ ori_gt_rt[j+1] @ q_homo[j].T)
            # npcs_q_homo中的每一个元组代表了一个轴点由于NPCS被相接的俩个part分裂成的俩个点
            npcs_q_homo.append((prev_q,next_q))  # ([4,],[4,])
        
        # print(npcs_q_homo[0][0],npcs_q_homo[0][1])
        # print(part_target_rt[0] @ npcs_q_homo[0][0],part_target_rt[1] @ npcs_q_homo[0][1])
        ...
        for i in range(self.num_parts):
            # clouds_part_cano[i] = o3d_inv_trans(clouds_part_cano[i],part_target_rt[i]) 
            if self.cfg.augment and not self.debug:
                clouds_part_cano[i] = pc_augment(clouds_part_cano[i])
            bounds_lst.append((np.max(clouds_part_cano[i],axis=0) - np.min(clouds_part_cano[i],axis=0)))
         
            clouds[i] = o3d_trans(clouds_part_cano[i], part_target_rt[i])  # 验证过,clouds和camera_clouds是一样的
            
            # np.savetxt('/home/xxx/codes/PPF_Tracker_release/pc_out.txt',camera_clouds[i])
            # np.savetxt('/home/xxx/codes/PPF_Tracker_release/pc_out1.txt',clouds[i])
  
        try:
            cloud = np.concatenate([clouds[i] for i in range(len(clouds))], axis=0)  # (num_pts, 3)       
            # camera_cloud = np.concatenate([camera_clouds[i] for i in range(len(camera_clouds))], axis=0)  # (num_pts, 3)       
            cloud_part_cano = np.concatenate([clouds_part_cano[i] for i in range(len(clouds_part_cano))], axis=0)
            cano_cloud = np.concatenate([cano_clouds[i] for i in range(len(cano_clouds))], axis=0)  # NAOCS 
            cano_cloud -= (np.max(cano_cloud,axis=0)+np.min(cano_cloud,axis=0)) / 2    
        except:
            # print(clouds)
        ...
        out_dic = {}
        out_dic['clouds'] = np.stack(clouds).astype(np.float32)  # 观测点云
        out_dic['clouds_part_cano'] = np.stack(clouds_part_cano).astype(np.float32)  # 标准化点云
        # np.savetxt('/home/xxx/codes/PPF_Tracker_release/pc_out.txt',np.concatenate(out_dic['clouds_part_cano'],axis=0))
        out_dic['part_target_rt'] = part_target_rt    # [numparts,4,4]
        out_dic['part_target_r'] = part_target_rt[:,:3,:3]
        out_dic['part_target_t'] = part_target_rt[:,:3,-1]
        out_dic['targets_scale'] = np.concatenate(bounds_lst,axis=-1,dtype=np.float32)  # 拼接成了[numparts*3,]
        out_dic['homo_q'] = np.stack(npcs_q_homo).astype(np.float32)  # [numparts-1,2,4]
        ...
        out_dic['cloud'] = cloud.astype(np.float32)  
        ...
        if self.cfg.seg_random:
            ...
            cls_pts = cano_cloud[cls_mask]
            cls_label = label[cls_mask]
        else:
            cls_pts = cano_cloud
            cls_label = label
        out_dic['cls_pts'] = cls_pts.astype(np.float32)
    ...
    def __getitem__(self, index):    
        ...
        res = self.get_frame(choose_frame_annotation, choose_urdf_id)
...
@hydra.main(config_path='/home/xxx/codes/PPF_Tracker_release/config', config_name='config')
def main(cfg):
    ...
    for i, data in enumerate(data_loader):
        d = data
        # data['homo_q'].shape = torch.Size([1, 3, 2, 4]) Dataloader出来之后会多出来一个batchsize的维度
