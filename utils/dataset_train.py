...
sys.path.append('/home/mxh24/codes/PPF_Tracker_release/utils')
...
class PM_Dataset(data.Dataset):  
    ...
    def get_fs_net_scale_part(self, c, urdf_id):
        ...
        # scale residual
        return residual_size.reshape(-1)*0.3, mean_shape_part.reshape(-1)*0.3 ,mean_shape*0.3 ,joint_loc, joint_axis
    @staticmethod
    ...
    def get_frame(self, choose_frame_annotation, choose_urdf_id, norm_joint_loc, norm_joint_axis):
        ...
        quasi_clouds = copy.deepcopy(canonical_clouds)  
        for i in range(self.num_parts):
           
            quasi_pcd = o3d.geometry.PointCloud() 
            quasi_pcd.points = o3d.utility.Vector3dVector(quasi_clouds[i]) 
        
            quasi_pcd.transform(part_target_rt[i])   
            if self.cfg.centric=='base':
                quasi_pcd.transform(np.linalg.inv(part_target_rt[0]))
            else:
                quasi_pcd.transform(np.linalg.inv(part_target_rt[1]))
            clouds[i] = np.asarray(quasi_pcd.points)
            
        centered_clouds = copy.deepcopy(canonical_clouds)
        for i in range(self.num_parts):
            centered_clouds[i] -= self.urdf_centers[str(choosen_urdf_id)][i]
        ...
        # pc = np.concatenate([pc,np.array([[1,1,1]])],axis=0)  
        ...
        # indices_base = indices[indices < clouds[0].shape[0]]
        # indices_child = indices[indices >= clouds[0].shape[0]]
        # pc_base = pc[indices_base]
        # pc_child = pc[indices_child]
        # pc_all = pc[indices]
        
        pc_base = copy.deepcopy(canonical_clouds[0])
        pc_child = copy.deepcopy(canonical_clouds[1])
        pc_base -= (np.min(pc_base, 0) + np.max(pc_base, 0)) / 2
        pc_child -= (np.min(pc_child, 0) + np.max(pc_child, 0)) / 2
        pc_base = pc_augment(pc_base)
        pc_child = pc_augment(pc_child) # 这俩验证了没问题，是NPCS
        
        
        # pc_all = np.concatenate((pc_base, pc_child), axis=0)
        if self.cfg.centric == 'base':
           
            pc_out = pc_base  
           
            s_ = np.array(self.cfg.scale_mean_base)
        else:
            pc_out = pc_child
            
            s_ = np.array(self.cfg.scale_mean_child)
        
       
        pc_out = pc_out + np.clip(self.cfg.res / 4 * np.random.randn(*pc_out.shape), -self.cfg.res / 2, self.cfg.res / 2)
        bounds = [np.min(pc_out,axis=0), np.max(pc_out,axis=0)]
        ...
        c_len = pc_out.shape[0]

        if c_len < 100 or len(pc_base)<100 or len(pc_child)<100:
            # input('BUG: c_len < 100 !')
        ...
        if c_len > self.cfg.npoint_max:  
            c_mask = np.random.choice(np.arange(c_len), self.cfg.npoint_max, replace=False)
            pc_out = pc_out[c_mask]
        
        
        pcd = o3d.geometry.PointCloud()   
        pcd.points = o3d.utility.Vector3dVector(pc_out)
        pcd.estimate_normals(search_param=o3d.geometry.KDTreeSearchParamKNN(knn=self.cfg.knn))
        normals = np.array(pcd.normals).astype(np.float32)

        # generate voting target
        center = np.array([0.,0.,0.])
        targets_tr, targets_rot, targets_rot_aux, point_idxs = generate_target(pc_out, normals, self.cfg.up_sym, self.cfg.right_sym, self.cfg.z_right, 200000, center)
        # [pairs, 2]  [pairs, 2]  
        ...
        #### use to segmentation
        label = np.zeros((len(pc_base)+len(pc_child)),dtype=np.int64)
        label[:len(pc_base)] = 0
        label[len(pc_base):] = 1
        cls_label = label
        cls_camera_pts = camera_cloud
        

        cls_random = True if self.cfg.centric == 'seg_random' else False
        if cls_random:
            cls_mask = np.random.choice(np.arange(len(label)), len(label), replace=False)
            cls_camera_pts = camera_cloud[cls_mask]
