...
"""
关键帧动态更新策略：如果当前帧frame1和camera_pts的hausdorff距离太大，则认为是关键帧是选取不好导致的，则之后应该重新选取关键帧
选取措施：可以记录前几帧的hausdorff距离以及对应的变换矩阵T2,如果当前的hausdorff比之前用的关键帧好，则把关键帧换成现在这帧
"""
...
def energy_function(point_cloud1:np.ndarray,point_cloud2:np.ndarray):
    ...
    c_mask = sample_points(point_cloud1,512,True)
...
def save_pts(pc0:np.ndarray, path=None):
    ...
    pc = pc[c_mask]
    if path == None:
        np.savetxt('/home/mxh24/codes/PPF_Tracker_release/pc_out.txt',pc)
    else:
...
def sample_points(points,num_pts=1024,get_mask=False):
    ...
    if c_len < num_pts:
        replace = True
    c_mask = np.random.choice(np.arange(len(points)), num_pts, replace=replace)
    if get_mask:
        return c_mask
