...
def rotation_matrix_exp(w):
    """
    Rodrigues' formula for exponential map from so(3) to SO(3)
    w: [n, 3, 3] or [3, 3]
    returns: [n, 3, 3] or [3, 3]
    """
    single = False
    if w.ndim == 2:
        w = w.unsqueeze(0)
    ...
    # θ = ||ω||, where ω is axis-angle vector
    omega = torch.stack([
        w[:, 2, 1] - w[:, 1, 2],
        w[:, 0, 2] - w[:, 2, 0],
        w[:, 1, 0] - w[:, 0, 1]
    ...
    I = torch.eye(3, device=w.device).unsqueeze(0)  # [1, 3, 3]
    A = torch.where(theta > 1e-6, torch.sin(theta) / theta, torch.ones_like(theta))
    B = torch.where(theta > 1e-6, (1 - torch.cos(theta)) / (theta ** 2), torch.zeros_like(theta))

    w_square = torch.matmul(w, w)
    exp_w = I + A * w + B * w_square  # [n, 3, 3]

    return exp_w.squeeze(0) if single else exp_w
